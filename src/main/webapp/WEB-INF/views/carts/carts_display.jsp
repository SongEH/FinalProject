    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px">
      ...
      <tbody>
        <c:forEach var="item" items="${list}">
          <c:if test="${item.shop_id != currentShopId}">
            ...
            <c:if test="${currentShopId != '' && shop_status=='영업중'}">
              <p>
                총 가격:
                <fmt:formatNumber
                  value="${currentShopTotal}"
                  pattern="#,###"
                />원
                <button
                  class="order-button button_style"
                  onclick="orderFromShop('${currentShopId}', '${currentShopName}')"
                >
                  주문
                </button>
              </p>
          ...
          <tr>
            ...
            <td style="padding: 10px; text-align: center">
              <div class="quantity-controls">
                <button
                  class="button_style small_btn"
                  onclick="updateQuantity('${item.carts_id}', 'minus')"
                >
                  -
                </button>
                ...
                <button
                  class="button_style small_btn"
                  onclick="updateQuantity('${item.carts_id}', 'plus')"
                >
                  +
                </button>
              </div>
            ...
            <td style="padding: 10px; text-align: right">
              <fmt:formatNumber
                value="${item.carts_quantity * item.menu_price}"
                pattern="#,###"
              />원
            </td>
            <td style="padding: 10px; text-align: center">
              <button
                class="button_style small_btn"
                onclick="deleteItem('${item.carts_id}')"
              >
                x
              </button>
            </td>
          ...
          <!-- 가게별 총 가격 계산 -->
          <c:set
            var="currentShopTotal"
            value="${currentShopTotal + (item.carts_quantity * item.menu_price)}"
          />
          <c:set
            var="totalPrice"
            value="${totalPrice + (item.carts_quantity * item.menu_price)}"
          />
